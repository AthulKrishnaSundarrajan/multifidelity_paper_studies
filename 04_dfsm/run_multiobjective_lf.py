...
desvars = {'omega_pc' : np.array([0.25]),'zeta_pc': np.array([2.50])}
...
class Model(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output('avg_pitch_travel',val = 0.)
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['avg_pitch_travel'] = model_outputs['avg_pitch_travel']
...
i_dv = 0
for key in desvars:
    model.add_design_var(key, lower=bounds[i_dv,0], upper=bounds[i_dv,1])
    i_dv+=1

model.add_constraint('GenSpeed_Max', upper=1.2)
model.add_objective('TwrBsMyt_DEL', ref=1.e5)
model.add_objective('avg_pitch_travel', ref=1)
